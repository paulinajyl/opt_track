...
NAME, APPLICATION_DATE, APPROVAL_DATE, CARD_PRODUCED_DATE, CARD_SHIPPED_DATE, CARD_DELIVERED_DATE = range(6)
...
async def receive_approval_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text("When was your EAD card isssued? Enter the date (YYYY-MM-DD) or type 'pending':")
        return CARD_PRODUCED_DATE
    except ValueError:
        await update.message.reply_text("Invalid format. Use YYYY-MM-DD or type 'pending'.")
        return APPLICATION_DATE

async def receive_card_produced_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text("When was your EAD card shipped? Enter the date (YYYY-MM-DD) or type 'pending':")
        return CARD_SHIPPED_DATE
    except ValueError:
        await update.message.reply_text("Invalid format. Use YYYY-MM-DD or type 'pending'.")
        return CARD_PRODUCED_DATE
        
async def receive_card_shipped_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    card_text = update.message.text
    ...
    if card_text.lower() == "pending":
        ...
        return ConversationHandler.END
    try:
        context.user_data['card_shipped_date'] = datetime.strptime(card_text, "%Y-%m-%d").date()
        await update.message.reply_text("When was your EAD card delivered? Enter the date (YYYY-MM-DD) or type 'pending':")
        return CARD_DELIVERED_DATE
    except ValueError:
        await update.message.reply_text("Invalid format. Use YYYY-MM-DD or type 'pending'.")
        return CARD_SHIPPED_DATE


async def receive_card_delivered_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    card_text = update.message.text
    ...
    try:
        context.user_data['card_delivered_date'] = datetime.strptime(card_text, "%Y-%m-%d").date()
        await save_application(update, context)
        return ConversationHandler.END
    except ValueError:
        await update.message.reply_text("Invalid format. Use YYYY-MM-DD or type 'pending'.")
...
async def save_application(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    conn = get_connection()
    cursor = conn.cursor()
    print((context.user_data['application_date'], context.user_data['approval_date'], context.user_data['card_produced_date'], context.user_data['card_shipped_date'], context.user_data['card_delivered_date'], update.effective_user.id))
    cursor.execute(
        "UPDATE applications SET application_date = %s, approval_date = %s, card_produced_date = %s, card_shipped_date = %s, card_delivered_date = %s WHERE user_id = %s",
        (context.user_data['application_date'], context.user_data['approval_date'], context.user_data['card_produced_date'], context.user_data['card_shipped_date'], context.user_data['card_delivered_date'], update.effective_user.id)
    )
...
add_new_opt = ConversationHandler(
    ...
    states={
        ...
        CARD_PRODUCED_DATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_card_produced_date)],
        CARD_SHIPPED_DATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_card_shipped_date)],
        CARD_DELIVERED_DATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_card_delivered_date)],
    },
